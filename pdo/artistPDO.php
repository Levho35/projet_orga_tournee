...
class ArtistPDO
{
    ...
    public function getIdGroup(int $id_artist): int
    {
        $MySQLQuery = 'SELECT id_group FROM `artist` WHERE id_artist = ?;';
        $stmt = DBConnection::getInstance()->prepare($MySQLQuery);
        $stmt->execute([$id_artist]);
        // Fetch l'ID du groupe depuis le résultat
        $result = $stmt->fetch();
        ...
        if ($result) {
            ...
            // Si l'artiste avec cet ID n'existe pas, tu peux retourner une valeur par défaut ou lancer une exception selon tes besoins
            // Exemple avec une valeur par défaut de -1
            return -1;
        }
    }
}
